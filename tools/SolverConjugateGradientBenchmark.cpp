...
int main(int argc, char* argv[]) {
    ...
    datafile << "NEW ENTRY OF BENCHMARK DATA OF CONJUGATE GRADIENT SOLVER" << std::endl;
    ...
    kfsoleq::soleq_float h = (1.0 / ((kfsoleq::soleq_float)poisson_inner_grid_size + 1));
    ...
    size_t solver_max_iters = 8192 * 16;
    ...
    for (solver_tolerance = max_solver_tolerance; solver_tolerance >= min_solver_tolerance; solver_tolerance -= solver_tolerance_step) {
        ...
        for (size_t iter_num = 0; iter_num < iters_block_size; ++iter_num) {
            roots = kfsoleq::solverConjugateGradient(solver_tolerance,
